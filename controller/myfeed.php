<?php
	function getmyFeed($conn){
		...
		$user = $_SESSION['uid'];
			...
			echo "<div class = 'feed-container' id='userfeed'><!-- Feed Conatiner-->";


//	    $loop = mysqli_query($conn,"SELECT * FROM posts ORDER BY postId DESC") or die (mysqli_error());
			$loop = mysqli_query($conn,"SELECT p.*, sum(ld.liked) as likes, sum(ld.disliked) as dislikes FROM posts p LEFT JOIN likes_dislikes ld ON p.postId = ld.pId WHERE uid = $user GROUP BY p.postid ORDER BY postId DESC") or die (mysqli_error());
