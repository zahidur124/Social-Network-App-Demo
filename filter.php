...
include "connector.php";
				...
				###################Channel Name Fetch#############################
				$cname_fetch_sql = mysqli_query($conn, "SELECT * FROM channels where cname ='$channel'   ") or die (mysqli_error($conn));
				$cname_array = mysqli_fetch_array($cname_fetch_sql);
				$cname_id = $cname_array['channelID'];
				###################Channel Name Fetch#############################


				###################Post Id Fetch#############################
				$cid_fetch_sql = mysqli_query($conn, "SELECT * FROM channel_posts where channelid ='$cname_id'   ") or die (mysqli_error($conn));
				...
				while(	$cid_array = mysqli_fetch_array($cid_fetch_sql)) {
					$channel_pid = $cid_array['pid'];

				$loop = mysqli_query($conn,"SELECT p.*, sum(ld.liked) as likes, sum(ld.disliked) as dislikes FROM posts p LEFT JOIN likes_dislikes ld ON p.postId = ld.pId WHERE postId ='$channel_pid' GROUP BY p.postid ORDER BY postId DESC") or die (mysqli_error($conn));
		  	$row = mysqli_fetch_array($loop);
				...
				$post_name_sql = mysqli_query($conn,"SELECT * FROM users WHERE userId = " . $post_admin . " ") or die (mysqli_error($conn));
